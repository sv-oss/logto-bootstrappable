...
import { autoGeneratedCustomDataClaims } from './auto-custom-data-claims.js';
...
export const createUserLibrary = (tenantId: string, queries: Queries) => {
  ...
  const insertUser = async (
    ...
    // Merge auto-generated custom data claims into customData.
    // Keys already set by the caller are never overwritten.
    const existingCustomData = data.customData ?? {};
    const generatedEntries = autoGeneratedCustomDataClaims
      .filter(({ key }) => !(key in existingCustomData))
      .map(({ key, generate }) => [key, generate()] as const);
    const customData = { ...existingCustomData, ...Object.fromEntries(generatedEntries) };

    const result = await pool.transaction<[User]>(async (connection) => {
      const user = await insertUserQuery({ ...data, customData });

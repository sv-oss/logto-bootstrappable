...
- **v1.41.0 integration:** Upstream v1.41 ships a full account-center redesign (multi-page layout with Sidebar/MobileTabNav, new Profile page with custom profile fields and AvatarUploadField, Sessions page, VerifiedAction page, social `change` mode, SSR theme injection, and `verifiedActionRoute`). The following decisions were made during the merge:
  - **Upstream takes precedence** for: `index.html` (SSR theme script), routing structure (Sidebar/full-page layout, `verifiedActionRoute`, social routes without dev feature flag, `hasSecurity`/`hasSessions`/`hasProfile` guards), Profile page (`index.tsx` and `index.module.scss` replaced by upstream's custom-field-based implementation), `account.ts` imports and function signatures (`updateProfile` now patches `/api/my-account/profile`, `updateCustomData` added).
  - **Fork behavior retained**: `PageHeader` rich user-dropdown (avatar, appearance submenu, sign-out) coexists with upstream's layout classes. `PageContextProvider` keeps `appearanceMode` state and localStorage persistence but adopts upstream's `getInitialTheme()` initializer and `!experienceSettings` early-return guard. `TotpBinding` redirects to `/authenticator-app/manage` instead of showing an error when TOTP already exists. `UpdateSuccess` preserves the "Back to account" button (`account_center.home.return_to_account`) when no `redirectUrl` is set.
  - **Retired**: Fork's simple Profile page (name/givenName/familyName/avatar form) replaced by upstream's custom profile field grid. `updateProfileFields` API function removed (upstream `updateProfile` covers the same endpoint). `account_center.profile.*` phrase keys removed from all locales (no longer referenced). `profileSuccessRoute` constant removed. Dev feature flag guard on social routes removed.
  - **Phrases**: 17 non-English locales had `profile` (fork) vs `sessions` (upstream) conflicts; `sessions` was taken. `account_center.profile.*` removed from English. Fork's `home.*` and `mfa.totp_manage_*` keys retained (still used by Home dashboard and TotpManage pages).
